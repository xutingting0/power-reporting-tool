    ...
    >
      ...
      <template #footer>
        ...
        <el-button
          ...
          @click="handleEditReport(selectedReport)"
        >
          编辑并重新提交
        </el-button>
      </template>
  ...
  setup() {
    ...
    const exportReports = async () => {
      try {
        ...
        link.href = url;
        link.download = `报备单_${new Date().getTime()}.xlsx`;
    ...
    const handleResubmit = async (row) => {
      try {
        ...
        // 将状态改为草稿，然后跳转到编辑页面
        await reportAPI.updateReport(row.id, { status: "draft" });
        ElMessage.success("已转为草稿，请修改后重新提交");
    ...
    const handleEditReport = (report) => {
      ...
      router.push({ path: "/user/create-report", query: { id: report.id } });
    };
